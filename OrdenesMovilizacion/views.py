...
class CrearRutaView(View):
    def post(self, request, id_usuario, *args, **kwargs):
        try:
            ...
            if RutasMovilizacion.objects.filter(ruta_descripcion=ruta_descripcion).exists():
                return JsonResponse({'error': 'La descripción de la ruta ya existe'}, status=400)
    ...
    def post(self, request, id_usuario, id_ruta_movilizacion, *args, **kwargs):
        try:
            ...
            try:
                ...
                if RutasMovilizacion.objects.filter(ruta_descripcion=ruta_descripcion).exclude(id_ruta_movilizacion=id_ruta_movilizacion).exists():
                    return JsonResponse({'error': 'La descripción de la ruta ya existe en otro registro'}, status=400)
...
@method_decorator(csrf_exempt, name='dispatch')
class CambiarEstadoRutaView(View):
    def post(self, request, id_usuario, id_ruta_movilizacion, *args, **kwargs):
        try:
            token = request.headers.get('Authorization')
            if not token:
            ...
            try:
                ...
                if ruta.ruta_estado == 1:
                    ruta.ruta_estado = 0  # Deshabilitar la ruta
                else:
                    ruta.ruta_estado = 1  # Habilitar la ruta
                
                ruta.save()

                estado_actual = "habilitada" if ruta.ruta_estado == 1 else "deshabilitada"
                return JsonResponse({'message': f'Ruta {estado_actual} exitosamente'}, status=200)
